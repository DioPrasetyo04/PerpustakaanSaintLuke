...
class LaporanBuku extends Page implements HasForms
{
    ...
    protected string $view = 'filament.pages.laporan-buku';
    ...
    public function getReportData(): array
    {
        ...
        $rows = Book::query()
            ...
            ->whereBetween('created_at', [$period['rangeStart'], $period['rangeEnd']])
            ->orderBy('created_at')
            ->get();

        $categoryStats = Category::withCount(['books' => function ($q) use ($period) {
            $q->whereBetween('books.created_at', [$period['rangeStart'], $period['rangeEnd']]);
        }])
            ->having('books_count', '>', 0)
            ->orderByDesc('books_count')
            ->get();

        $publisherStats = Publisher::withCount(['books' => function ($q) use ($period) {
            $q->whereBetween('created_at', [$period['rangeStart'], $period['rangeEnd']]);
        }])
            ->having('books_count', '>', 0)
            ->orderByDesc('books_count')
            ->get();

        $byStatus = $rows->groupBy(fn($b) => $b->status instanceof \BackedEnum
            ? $b->status->value
    ...
    public function downloadPdf()
    {
        ...
        if ($report['total'] === 0) {
            Notification::make()
                ...
                ->body('Tidak ada data buku pada periode yang dipilih.')
                ->warning()
                ->send();
            return null;
        }

        $chartTrend = $this->buildBarChart($report['buckets'], '#3b82f6');

        $categoryBuckets = $report['categoryStats']->map(fn($c) => [
            'label' => $c->name,
            'range' => $c->name,
            'value' => $c->books_count,
        ])->toArray();

        $publisherBuckets = $report['publisherStats']->map(fn($p) => [
            'label' => $p->name,
            'range' => $p->name,
            'value' => $p->books_count,
        ])->toArray();
