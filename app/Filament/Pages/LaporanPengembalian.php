...
class LaporanPengembalian extends Page implements HasForms
{
    ...
    public function mount(): void
    {
        $this->form->fill([
            ...
            'week'  => '1',
            'judul' => 'LAPORAN PENGEMBALIAN BUKU PERPUSTAKAAN SANTO LUKAS',
            'nomor' => '001/LAP-RTB/' . now()->format('m') . '/' . now()->format('Y'),
            'kota'  => 'Jakarta',
    ...
    public function getReportData(): array
    {
        ...
        return [
            ...
            'rows'         => $rows,
            'total'        => $rows->count(),
            'totalFine'    => $rows->sum(fn($r) => optional($r->fine)->total_fee ?? 0),
        ];
    }
    ...
    public function downloadPdf()
    {
        ...
        if ($report['total'] === 0) {
            Notification::make()
                ...
                ->body('Tidak ada data pengembalian pada periode yang dipilih.')
                ->warning()
                ->send();
            return null;
        }
