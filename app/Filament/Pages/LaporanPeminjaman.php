...
class LaporanPeminjaman extends Page implements HasForms
{
    ...
    public function getReportData(): array
    {
        ...
        $borrowed = $rows->where('status', LoanBookStatus::BORROWED)->count();
        $returned = $rows->where('status', LoanBookStatus::RETURNED)->count();
    ...
    public function downloadPdf()
    {
        ...
        if ($report['total'] === 0) {
            Notification::make()
                ...
                ->body('Tidak ada data peminjaman pada periode yang dipilih.')
                ->warning()
                ->send();
            return null;
        }
