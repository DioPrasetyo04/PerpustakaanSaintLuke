...
- [ ] `https://domain-anda.com` tampil (frontend), `/admin` bisa login.
- [ ] `APP_DEBUG=false`, `APP_ENV=production`.
- [ ] Aset (CSS/JS) termuat → `public/build` ada & `npm run build` terbaru.
- [ ] `storage:link` aktif → cover buku, kartu PDF, file tampil.
- [ ] Registrasi/login + verifikasi email berfungsi (cek inbox).
- [ ] Login Google berhasil (redirect URI cocok).
- [ ] `php artisan schedule:list` menampilkan 2 task hourly.
- [ ] Tunggu/test cron → reminder email & WhatsApp terkirim (cek `storage/logs/laravel.log`).
- [ ] Auto-return digital expired berjalan.
- [ ] Pembayaran denda Midtrans (mode sesuai) sukses.
- [ ] (Jika dipakai) konversi Office→PDF jalan, atau buku digital diunggah sebagai PDF langsung.
- [ ] `storage/logs/laravel.log` tidak berisi error fatal.
