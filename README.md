...
1. **Cron Job di cPanel** (menu _Cron Jobs_), tiap menit:

    ```
    * * * * * /usr/local/bin/php /home/USER/perpustakaan/artisan schedule:run >> /dev/null 2>&1
    ```

    > Ganti `USER` & path. Path PHP cPanel sering `/usr/local/bin/php` atau pakai versi spesifik (`ea-php82`), cek di _Select PHP Version_ / _MultiPHP_.
...
**Via phpMyAdmin (Laragon → Database):** pilih database → tab **Export** → format **SQL** → _Go_.
...
2. Buka **phpMyAdmin** → pilih database baru → tab **Import** → unggah `backup_db.sql` → _Go_.
...
- **Opsi A (disarankan):** set _Document Root_ domain ke `/home/USER/perpustakaan/public` (lewat _Domains_ / addon domain).
